...
export interface StyleMatch {
  category: StyleCategory;
  matchedOption: StyleOption | null;
  originalDescription: string;
  matchConfidence: number; // 0-100
  isAvailable: boolean; // Whether the matched option exists in our library
}

export interface StyleSuggestion {
  category: StyleCategory;
  description: string;
  reason: string; // Why this style isn't available / what makes it unique
}

export interface StyleAnalysisResult {
  matches: StyleMatch[];
  suggestions: StyleSuggestion[]; // Styles detected but not available in app
  overallVibe: string; // e.g., "Glamorous Hollywood", "E-Girl Aesthetic", "Natural Boho"
  rawAnalysis: {
    hairStyle?: string;
    hairColor?: string;
    hairLength?: string;
